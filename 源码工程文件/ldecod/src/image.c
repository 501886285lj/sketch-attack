 ...
 ************************************************************************
 */
void downSample(IplImage **edge){			//下采样16倍以和sketch统一尺寸
	int r,c,i;
	int width = (*edge)->width;
	int height = (*edge)->height;
	int down_width = (width+15)/16;
	int down_height = (height+15)/16;

	IplImage *tmp = cvCreateImage(cvSize(down_width,down_height),8,1);

	//初始化全0
	for(i = 0;i<down_height*down_width;i++)
		tmp->imageData[i] = 0;

	//下采样
	for(r = 0;r<height;r++)
		for(c = 0;c<width;c++)
			if((*edge)->imageData[r * width + c])
				tmp->imageData[r/16 * down_width + c/16] = 255;

	cvReleaseImage(edge);
	(*edge) = tmp;
}

void attack(VideoParameters *p_Vid,Slice *currSlice,int *data,int ms,int sr){
	IplImage *img,*sr_img;
	int i,max = 0;
	int height = p_Vid->height/16;
	int width = p_Vid->width/16;
	...
	//构造图片输出路径
	strcpy(subRoute,route);
	strcat(subRoute,"/frame");
	itoa(frame_id,id,10);
	strcat(subRoute,id);
	strcat(subRoute,".jpg");
	...
	//赋值
	img = cvCreateImage(cvSize(width,height),8,1);	//8深度1通道
	for(i = 0;i<count;i++)
		img->imageData[i] = data[i];

	//超分
	sr_img = cvCreateImage(cvSize(width * sr,height * sr),8,1);	//8深度1通道
	cvResize(img,sr_img,CV_INTER_CUBIC);						//进行超分，用CUBIC方法

	//将图片存储下来
	cvSaveImage(subRoute,sr_img,0);

	//将图片显示出来
	cvNamedWindow("框架攻击      SCUT梁剑 E-mail:doc.liang@qq.com", CV_WINDOW_AUTOSIZE);
	cvShowImage("框架攻击      SCUT梁剑 E-mail:doc.liang@qq.com",sr_img);
	cvWaitKey(ms);

	//如果有yuv输入，则计算ESS标量
	if(yuv != NULL){
		//计算视频尺寸
		IplImage *edgeOriginal,*edgeSketch,*origImg;
		StorablePicture *p = p_Vid->dec_picture;
		static const int SubWidthC  [4]= { 1, 2, 2, 1};
		static const int SubHeightC [4]= { 1, 2, 1, 1};
		int crop_left, crop_right, crop_top, crop_bottom;
		int iLumaSizeX,iLumaSizeY;
		int r,c;
		int b = 0,w = 0,n1 = 0,n2 = 0,n3 = 0,n4 = 0;
		double ess;
		uchar *buff;
		char cannyRoute[255];
		...
		frameSize = iLumaSizeX * iLumaSizeY;
		buff = (uchar *)malloc(frameSize * sizeof(uchar));

		origImg = cvCreateImage(cvSize(iLumaSizeX,iLumaSizeY),8,1);
		edgeOriginal = cvCreateImage(cvSize(iLumaSizeX,iLumaSizeY),8,1);
		edgeSketch = cvCreateImage(cvSize(width,height),8,1);

		fseek(yuv,frameSize * 3 / 2 * frame_id,0);		//设置文件指针偏量
		fread(buff,sizeof(uchar),frameSize,yuv);		//读取Y通道的数据
		memcpy(origImg->imageData,buff,frameSize * sizeof(uchar));		//赋值数据

		//使用canny detector
 		cvCanny(origImg,edgeOriginal,minthresold,maxThresold,cannySize);
		cvThreshold(img,edgeSketch,0,255,CV_THRESH_OTSU);

		strcpy(cannyRoute,route);
		strcat(cannyRoute,"/canny");
		strcat(cannyRoute,id);
		strcat(cannyRoute,".jpg");

		//将下采样后的canny存储下来
		cvSaveImage(cannyRoute,edgeOriginal,0);

		//对canny detector构造出来的边缘图像进行下采样
		downSample(&edgeOriginal);

		//统计B/W/n1/n2/n3/n4
		for(r = 0;r<height;r++)
			for(c = 0;c<width;c++){
				//统计B/W
				if(edgeOriginal->imageData[r * width + c])
					w++;
				else
					b++;

				//统计n1 n2 n3 n4
				if(!edgeOriginal->imageData[r * width + c] && !edgeSketch->imageData[r * width + c]){
					n1++;
					continue;
				}
				if(edgeOriginal->imageData[r * width + c] && edgeSketch->imageData[r * width + c]){
		...
		//计算ess并写文件
		ess = fabs( n1*1.0/(2*b) + n2*1.0/(2*w) - n3*1.0/(2*b) - n4*1.0/(2*w) );

		switch(currSlice->slice_type){
		case I_SLICE:fprintf(essFile,"%d,Islice,%f\n",frame_id,ess);break;
		case P_SLICE:fprintf(essFile,"%d,Pslice,%f\n",frame_id,ess);break;
		case B_SLICE:fprintf(essFile,"%d,Bslice,%f\n",frame_id,ess);break;
		}

		mean += ess;

// 		cvShowImage("originalEdge",edgeOriginal);
// 		cvShowImage("sketchEdge",edgeSketch);

		cvReleaseImage(&origImg);
		cvReleaseImage(&edgeSketch);
		cvReleaseImage(&edgeOriginal);
		free(buff);
	}

	//释放图片
	cvReleaseImage(&img);
	cvReleaseImage(&sr_img);
...
void decode_one_slice(Slice *currSlice)
{
  ...
  int offset = 0;								//记录之前的bitstream 的偏移量
  int before;
  int *imgData = (int *)malloc(sizeof(int) * p_Vid->height * p_Vid->width / 256);		//申请sketch内存
  ...
  while (end_of_slice == FALSE) // loop over macroblocks
  {
	...
	before = currSlice->partArr[assignSE2partition[currSlice->dp_mode][SE_MBTYPE]].bitstream->frame_bitoffset;			//取bits之前的offset
    currSlice->read_one_macroblock(currMB);
	imgData[currMB->mbAddrX] = currSlice->partArr[assignSE2partition[currSlice->dp_mode][SE_MBTYPE]].bitstream->frame_bitoffset-before;		//取bits之后的offset，记录差即为currMB的 bits
  ...
  //reset_ec_flags(p_Vid);
	attack(p_Vid,currSlice,imgData,fps,sr);
